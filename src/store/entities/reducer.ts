...
import { LIST_ENTITIES_SUCCESS } from '@app/store/entities/operations/list/constants';
...
const reducer: Reducer<IEntity[], EntityActionTypes> = (state = [], action) => {
  switch (action.type) {
    ...
    case UPDATE_ENTITY_SUCCESS: {
      const { entity } = action.payload;
      const idx = state.map((item) => item.objectId).indexOf(entity.objectId);
      return [...state.slice(0, idx), entity, ...state.slice(idx + 1)];
    }
    case DELETE_ENTITY_SUCCESS: {
      const { id } = action.payload;
      const idx = state.map((item) => item.objectId).indexOf(id);
